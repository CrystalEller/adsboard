(function ($) {
    ...
    function GTreeTable(element, options) {
        ...
        '<div class="btn-group pull-right ' + this.options.classes.buttons + '">${editFormButton}${actionsButton}${actionsList}</div>' +
        '</td>' +
        '</tr>' +
        '</table>';
        ...
        this.templateParts = this.options.templateParts !== undefined ? this.options.templateParts :
        {
            ...
            editFormButton: '<button type="button" class="btn btn-sm btn-default edit-form">' + 'Редактирование формы' + ' </button>',
            actionsList: ''
        };
        ...
        if (this.actions.length > 0) {
            ...
            $.each(this.actions, function (index, action) {
                if (action.divider === true) {
                    ...
                    var hide = '';
                    if (matches[1] === 'createBefore') {
                        hide = 'style="display: none"';
                    }
                    templateActionsList += '<li role="presentation"><a href="#notarget"' + hide + 'class="node-action-' + index + '" tabindex="-1">' + name + '</a></li>';
                }
            });
    ...
    GTreeTableNode.prototype = {
        ...
        getParentNode: function () {
            return this.manager.getNodeById(this.getParent());
        },
        ...
        render: function () {
            ...
            if (!this.hasChildren) {
                this.$node.find('.' + this.manager.options.classes.ceIcon).hide();
                //this.$name.css('marginLeft', '20px');
            }

            if (!this.getParent()) {
                this.level = 0;
            } else {
                var parentNode = this.manager.getNodeById(this.getParent());
        ...
        attachEvents: function () {
            ...
            this.$node.on('click', 'button.edit-form', function (e) {
                $('#edit-form-modal .modal-dialog').width(0.8 * $(window).width());
                $('#edit-form-modal .modal-title').text('Категория: ' + that.name);
                $('#edit-form-modal').modal('show');
                $('#form-elements').empty().formBuilder(that.id);
            });
        ...
        getResponseMessage: function (data) {
            if (data.message) {
                return $.map(data.message, function (value, index) {
                    return value;
                });
            }
        },
        ...
        save: function () {
            ...
            if ($.isFunction(oNode.manager.options.onSave)) {
                $.when($.ajax(oNode.manager.options.onSave(oNode))).done(function (data) {
                    var message = oNode.getResponseMessage(data);
                    if (message) {
                        alert(message)
                    } else {
                        oNode._save(data);
        ...
        add: function (position, type) {
            ...
            this.$node.find('.' + this.manager.options.classes.ceIcon).show();
        ...
        _move: function (oDestination, position) {
            ...
            if (oOldNodeParent !== undefined && oOldNodeParent.getDescendants({
                    depth: 1,
                    includeNotSaved: true
                }).length === 0) {
            ...
            if (oSiblings.length > 0) {
                ...
                if (oRelated === undefined) {
                    ...
                    if (oRelated.isExpanded()) {
                        oRelated = oNode.manager.getNodeById(oNode.parent).getDescendants({
                            depth: -1,
                            index: -1,
                            includeNotSaved: true
                        });
        ...
        languages: {
            'en-US': {
                ...
                actions: {
                    ...
                    createFirstChild: 'Создать подкатегорию',
